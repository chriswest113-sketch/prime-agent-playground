...
| **Q2** Turn-level harness attribution | "Provenance exists" vs. "provenance is absent" | Eleven-dimension matrix over three real persisted sessions: `/refine` writer, CRUD writer, and a reverse-replay probe added after audit | TEST-CONFIRMED + RUNTIME-CONFIRMED | **Writer-purity-dependent.** `/refine`: tree position, ordering, before, after, message split, per-entry state — and, corrected after audit, **complete state at an earlier checkpoint**, via reverse-replay of the ordered edit log (T02.C1). Direct CRUD: none of those. One interleaved foreign write breaks reconstruction **silently** (T02.C2/C3). Neither writer supports recovering the system prompt actually sent | Global-scope refinement history (`refinements.jsonl`) was not exercised; reconstruction across compaction/branch/fork untested; whether a live RLM kernel adds provenance the bare Python API does not |
...
All probes exit non-zero on any failed assertion. Latest full run: **6/6 PASS**, 93 assertions.

| Probe | Assertions | Result |
| --- | --- | --- |
| `test-01-gate-refinement-coupling` | 10 | PASS |
| `test-02-turn-harness-attribution` | 18 | PASS |
...
9. **The probes were not type-checked in the first round.** `studies/` sits
   ...
   `studies/joint-discriminating-tests/tsconfig.json` surfaced seven type errors,
   one of them substantive: Test 05 passed `"standard"` as a service tier, which
   is not in the `ServiceTier` union. **That invalidated the original Q5
...
10. **An audit corrected a claim this study asserted without testing.** The
   ...
   log in fact reconstructs it **exactly** (T02.C1). The row is corrected, and
   the real failure mode — silent corruption under a foreign writer — is now
